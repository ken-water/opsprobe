...
Its goal is narrower than full installer acceptance. It proves that the current repository can produce real Linux desktop bundle candidates instead of stopping at `cargo check` or frontend build success.
...
- the artifact confirms:
  - `apps/desktop/src-tauri/target/release/bundle/deb/OpsProbe_0.10.3_amd64.deb`
  - `apps/desktop/src-tauri/target/release/bundle/rpm/OpsProbe-0.10.3-1.x86_64.rpm`
