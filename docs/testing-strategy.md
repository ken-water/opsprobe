...
- `0.11.1`: every pushed checkpoint on the active `0.11.x` line must already have a unique patch version and matching changelog entry
- `0.11.2`: every packaging or installer change must refresh Linux packaged validation artifacts and the Windows validation record
- `0.11.2`: once that version is active, real Windows installer acceptance must also produce one completed version-specific acceptance note on a Windows-capable environment before the issue is considered closed
- `0.11.3`: stable-decision docs must be backed by a fresh smoke run plus the current blocker table
- `0.11.3`: once that version is active, the blocker table must classify every remaining `1.0.0` risk as fixed, accepted, or deferred before any `1.0.0` planning starts
